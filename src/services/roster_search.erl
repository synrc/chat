...
info(#'Search'{id = From, ref = Ref, field = <<"phone">>, type = '==', value = Phones, status = contact}, Req, #cx{params = <<"emqttd_", _/binary>> = ClientId} = State) when is_list(Phones), Ref /=[] ->
info(#'Search'{} = Data, Req, State) -> {bert, #io{}}, Req, State}.
