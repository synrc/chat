...
`CHATBody.batch` у DSL і runner трактується як transport-обгортка,
а не як окрема доменна семантика.

Це означає:

- engine повинен розгорнути `batch` у впорядковану послідовність `packet`;
- порядок `packet` усередині `batch` зберігається;
- кожен `packet` далі обробляється так само, як `single`;
- помилка одного `packet` не скасовує сусідні;
- `batch` не створює спільного snapshot, спільного command context або atomic transaction.

DSL не описує, коли саме сервер або клієнт вирішують використовувати
`batch`. Це transport/runtime-рішення. На рівні executable semantics DSL
фіксує лише таке правило:

- якщо transport приніс `batch`, клієнтський engine розпаковує його
  у впорядкований потік `packet` і застосовує їх по одному,
  без нової предметної семантики.
