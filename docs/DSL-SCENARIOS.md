# DSL Scenarios

## Purpose

Короткий опис навіщо DSL.

## Structure

- [DSL-CORE.md](./DSL-CORE.md) — grammar, duality, expect semantics
- [DSL-AUTH.md](./DSL-AUTH.md) — auth/session lifecycle
- [DSL-READ.md](./DSL-READ.md) — read/cursor/multi-session/multi-feed
- [DSL-REPLAY.md](./DSL-REPLAY.md) — replay/gap/snapshot recovery
- [DSL-PAGINATION.md](./DSL-PAGINATION.md) — inbox pagination and event streaming
- [DSL-ADVANCED.md](./DSL-ADVANCED.md) — version, federation

## Coverage

- delivery
- auth
- read
- replay
- gap
- pagination
- version
- federation